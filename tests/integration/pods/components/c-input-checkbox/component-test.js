...
test('it renders', function(assert) {
  ...
  assert.equal(
    this.$().text().trim().indexOf('A Label') !== -1,
    true,
    'Label text'
  );

  assert.equal(
    this.$().find('input[type="checkbox"].e-input-checkbox').length,
    3,
    '3 input elements with type checkbox and class name e-input-checkbox'
  );

  assert.equal(this.$().find('.e-label').length, 4, '4 e-label elements');

  assert.equal(this.$().find('label.e-label--modifier').length, 1, 'label with labelClass');

  assert.equal(this.$().find('input.e-input-checkbox--modifier').length, 3, 'input with fieldClass');
});
