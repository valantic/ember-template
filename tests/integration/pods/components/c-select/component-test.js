...
test('it renders', function(assert) {
  ...
  assert.equal(this.$().find('select[name="c-select-1"]').length, 1,
      'select element with name c-select-1');
  assert.equal(this.$().find('select.e-select option').length, 3,
  ...
  assert.equal(this.$().find('select.e-select--modifier').length, 1,
      'select with fieldClass');
  assert.equal(this.$().find('select#theFieldId').length, 1,
      'select with id');
  assert.equal(this.$().find('select[size="3"]').length, 1,
      'select with size');
  assert.equal(this.$().find('select[multiple]').length, 1,
